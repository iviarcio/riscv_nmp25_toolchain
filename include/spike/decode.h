...
public:
  ...
  // Code Added for NMP 
  //------------------------------------------------------------- 
  unsigned int np0() { return x(25, 2); }  // returns Nparam0 from the instruction
  unsigned int np1() { return x(12, 3); }  // returns Nparam1 from the instruction
  //------------------------------------------------------------- 
...
// Code Added for NMP 
//------------------------------------------------------------- 
const int N_NMPCSR = 21;

// Used in executing NMP instructions 
#define NM_TILE p->get_sim()->get_tl(p->tl_id) 
#define NM_TLT ((p->get_id()) % (p->get_sim()->nproctl))  
#define GET_TILE(value) p->get_sim()->get_tl(value) 
#define P0  p->get_csr(CSR_NPARAM0)
#define P1  p->get_csr(CSR_NPARAM1)
#define P2  p->get_csr(CSR_NPARAM2)
#define P3  p->get_csr(CSR_NPARAM3)
#define P4  p->get_csr(CSR_NPARAM4)
#define CFG1  p->get_csr(CSR_NCONFIG1)
#define SET_SEMA(value) p->set_csr(CSR_NSEMA,(p->get_csr(CSR_NSEMA) | value))
#define MBLB(value) (((value) & 0x1)? 0 : ((value) & 0x2) ? 1 : ((value) & 0x4) ? 2 : 0)
// Used in processor.cc
#define CFGMB(value) sim->get_tl(tl_id)->cnfg_mb(((id) % (sim->nproctl)),value)
#define CFGFRMT(value) sim->get_tl(tl_id)->cnfg_frmt(((id) % (sim->nproctl)),value)
//------------------------------------------------------------- 
